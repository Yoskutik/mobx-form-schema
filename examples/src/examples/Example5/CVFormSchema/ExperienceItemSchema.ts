...
const stoppedWorking = (_: unknown, schema: ExperienceItemSchema) => !schema.stillWorking;
...
export class ExperienceItemSchema extends FormSchema {
  ...
  @validate(defined(), month()).if(stoppedWorking)
  @watch toMonth: number | null = null;

  @presentation((value, schema) => schema.stillWorking ? undefined : value)
  @validate(defined(), validateFromYear()).if(stoppedWorking)
