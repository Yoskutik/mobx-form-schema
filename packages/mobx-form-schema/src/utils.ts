...
export const updateMetadata = <T>(
  ...
  if (target && (isTypeOf(propertyKey) || isTypeOf(propertyKey, 'symbol'))) {
    ...
    (getMetadataFromMap(target) || (
      TargetToMedataMap.set(target, {}) && getMetadataFromMap(target)
...
export const warnAboutProperty = (schema: FormSchema, key: string, message: string) => {
  // eslint-disable-next-line no-console
  console.warn(
    `%c[Mobx Form Schema]%c Property %c"${key}"%c of schema %c${schema.constructor.name}%c ${message}`,
