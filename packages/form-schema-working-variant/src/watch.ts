...
import { COMPARATOR, getMetadata, OBJECT_KEYS, SCHEMA, SCHEMAS_ARRAY, updateMetadata } from './utils';
import type { FieldOrGetDecoratorThis, FieldOrGetDecorator, CreatePropertyDecoratorType } from './types';
...
export const checkSingleSchemaEquality = <This, Value extends FormSchema>(newSchema: Value, oldSchema: Value, schema: This, watchConfig?: any, isDifferent?: any) => {
...
const configure: Watch['configure'] = <This, Value>(comparator, saveFn, restoreFn) => {
  const decorator = (target, propertyKey) => (
    updateMetadata<TWatchConfig<This, Value>>(WatchSymbol, target, propertyKey, { [COMPARATOR]: comparator, saveFn, restoreFn })
  );
