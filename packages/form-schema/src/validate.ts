...
import { __ValidateSymbol } from './symbols';
...
export const validate: Validate = (...validators) => (
  (target, propertyKey) => (
    updateMetadata<TValidationConfig<any, any>>(__ValidateSymbol, target, propertyKey, { validators })
  )
);

validate.if = (condition, validators) => (
  (target, propertyKey) => (
    updateMetadata<TValidationConfig<any, any>>(__ValidateSymbol, target, propertyKey, { validators, condition })
  )
);
